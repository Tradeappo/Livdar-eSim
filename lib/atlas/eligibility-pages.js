...
import { ACTIVITY_IDS } from './sport.js';
...
export function sourceCoverage() {
  ...
  // Only the countries that carry earnings, not every country in the file.
  // The Eurostat set includes the United States, Turkey and Albania with a
  // statutory minimum wage and no gross or net series, and a salary page
  // needs one of those two to say anything at all. Counting the file's keys
  // instead put ten pages into the cohort that refused to build.
  if (sal) {
    cov['salary-data-verified'] = new Set(Object.entries(sal.countries)
      .filter(([, c]) => c.measures && (c.measures.grossMonthly || c.measures.netMonthly))
      .map(([iso2]) => iso2));
  }
  ...
  if (climate) {
    ...
    // Three levels of entity sit on this one source, because three families
    // ask different questions of it. A country best time page needs the
    // country, a city page needs the city, and a sport season page needs a
    // city and an activity together, written the way that page addresses it,
    // `1850147:running`. The same pattern as the holiday regions: a composite
    // entity is still one entity, and the coverage set has to contain the
    // string the family will look up.
    cov['climate-normals-verified'] = new Set([
      ...rows.map((c) => c.iso2).filter(Boolean),
      ...rows.map((c) => String(c.cityId)),
      ...rows.flatMap((c) => ACTIVITY_IDS.map((a) => String(c.cityId) + ':' + a)),
    ]);
  }
  ...
  // The public holiday source covers the countries whose files reach the
  // current year. A country whose dates stop last year cannot carry a page
  // about this one, and publishing last year's calendar would be worse than
  // publishing nothing.
  const holidays = readIf('data/atlas/sources/events/public-holidays.json');
  if (holidays) {
    // Countries and the regions inside them, because two families sit on this
    // source and their entities are at different levels. A region is written
    // the way the page addresses it, `DE-BY`, so that a family asking about
    // Bavaria and a family asking about Germany both find their own answer.
    const rows = Object.entries(holidays.store || {});
    cov['events-verified'] = new Set([
      ...rows.map(([iso2]) => iso2),
      ...rows.flatMap(([iso2, c]) => (c.subdivisions || []).map((s) => iso2 + '-' + s.shortName)),
    ]);
  }
