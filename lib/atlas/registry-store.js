...
export function migrate() {
  ...
  const ok = Object.keys(all).length === Object.keys(check).length && Object.keys(all).every((k) => check[k] && check[k].state === all[k].state);
  return { ...res, shards: REGISTRY_SHARDS, verified: ok };
}
