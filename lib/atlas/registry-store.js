...
// Remove entries outright. This is not a state transition and must not be
// used as one: a page that reached `published` has a history worth keeping
// even after it is withdrawn, and `retired` exists for that.
//
// It is for un-claiming. `approved` means scoring cleared the page and a lot
// claimed it, so an entry whose lot no longer contains it is not approved, it
// is a leftover from a cohort that was rebuilt. Leaving it makes the funnel
// count pages no cohort will ever publish.
export function deleteEntries(keys) {
  const touched = new Map();
  for (const key of keys) {
    const n = registryShard(key);
    if (!touched.has(n)) touched.set(n, { ...readShardFile(n) });
    delete touched.get(n)[key];
  }
  for (const [n, obj] of touched) {
    const sorted = Object.fromEntries(Object.entries(obj).sort());
    writeFileSync(new URL('registry/' + shardFile(n), ROOT), JSON.stringify(sorted, null, 1) + '\n');
  ...
  return { shardsTouched: touched.size, entriesRemoved: keys.length };
}
