...
const OVERRIDES = {
  ...
  fr: {
    AU: "l'Australie", BR: 'le Brésil', CN: 'la Chine', CV: 'le Cap-Vert', KR: 'la Corée du Sud',
    MU: 'Maurice', PH: 'les Philippines',
  ...
  it: {
    AR: "l'Argentina", AU: "l'Australia", BR: 'il Brasile', CN: 'la Cina', GE: 'la Georgia',
    PA: 'Panama', RU: 'la Russia', SG: 'Singapore', TN: 'la Tunisia',
  ...
  pt: {
    AR: 'a Argentina', BR: 'o Brasil', CL: 'o Chile', PY: 'o Paraguai', UY: 'o Uruguai',
...
// Country names that appear in measured keywords and are not the CLDR name.
//
// A heading is built from the keyword, and the keyword carries whatever the
// market actually searches. Italian searches `repubblica ceca` where CLDR says
// `Cechia`, so the name is not found and the fallback raises only the last
// token, giving `repubblica Ceca`. Single word names never had this problem,
// which is why it surfaced only when a cohort first reached a country whose
// common name is two words.
//
// Only multi word alternates belong here. A single word alternate is already
// handled correctly by raising the last token.
const KEYWORD_ALIASES = {
  it: { 'repubblica ceca': 'Repubblica Ceca', 'costa rica': 'Costa Rica', 'stati uniti': 'Stati Uniti', 'corea del sud': 'Corea del Sud', 'nuova zelanda': 'Nuova Zelanda', 'paesi bassi': 'Paesi Bassi' },
  fr: { 'republique tcheque': 'République tchèque', 'république tchèque': 'République tchèque', 'costa rica': 'Costa Rica', 'cap vert': 'Cap-Vert', 'coree du sud': 'Corée du Sud', 'corée du sud': 'Corée du Sud', 'ile maurice': 'Maurice', 'etats unis': 'États-Unis', 'pays bas': 'Pays-Bas', 'nouvelle zelande': 'Nouvelle-Zélande' },
  es: { 'republica checa': 'República Checa', 'república checa': 'República Checa', 'reino unido': 'Reino Unido', 'republica dominicana': 'República Dominicana', 'estados unidos': 'Estados Unidos', 'nueva zelanda': 'Nueva Zelanda', 'paises bajos': 'Países Bajos', 'costa rica': 'Costa Rica', 'corea del sur': 'Corea del Sur', 'arabia saudita': 'Arabia Saudita' },
  pt: { 'republica checa': 'República Checa', 'reino unido': 'Reino Unido', 'estados unidos': 'Estados Unidos', 'costa rica': 'Costa Rica', 'coreia do sul': 'Coreia do Sul' },
  de: { 'vereinigtes koenigreich': 'Vereinigtes Königreich' },
  nl: { 'verenigd koninkrijk': 'Verenigd Koninkrijk' },
  en: { 'czech republic': 'Czech Republic', 'united kingdom': 'United Kingdom', 'united states': 'United States', 'costa rica': 'Costa Rica' },
  pl: {},
  ja: {},
};

// The alternates for a language, longest first so that the longest match wins
// and a shorter name inside a longer one cannot claim it.
export function keywordAliases(language) {
  return Object.entries(KEYWORD_ALIASES[language] || {}).sort((a, b) => b[0].length - a[0].length);
}
