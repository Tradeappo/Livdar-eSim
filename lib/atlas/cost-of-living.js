...
// The countries either side of this one on its own scale.
//
// A country measured only by the World Bank ratio has almost nothing on its
// page that is its own: a value, a year and a rank. Two such countries with
// the same value produce two pages that differ by a name, which is how
// Argentina and Brazil came out 83 percent alike. Naming the neighbours fixes
// that with measured data rather than with padding, and it answers the
// question a rank raises and does not settle, which is what the country is
// actually next to.
//
// Both neighbours come from the same scale as the country itself, because the
// Eurostat index and the World Bank ratio are not comparable and a neighbour
// drawn from the other one would be a number nobody measured.
export function neighbours(iso2, { measure = 'A01' } = {}) {
  const self = store().countries[iso2];
  const own = self?.measures?.[measure] || (self?.headlineMeasure ? self.measures[self.headlineMeasure] : null);
  if (!own) return null;
  const rows = [];
  for (const [id, c] of Object.entries(store().countries)) {
    const r = c.measures[measure] || (c.headlineMeasure ? c.measures[c.headlineMeasure] : null);
    if (!r || r.unit !== own.unit) continue;
    rows.push({ iso2: id, value: r.value });
  }
  rows.sort((a, b) => a.value - b.value || a.iso2.localeCompare(b.iso2));
  const i = rows.findIndex((r) => r.iso2 === iso2);
  if (i < 0) return null;
  return {
    unit: own.unit,
    of: rows.length,
    rankCheapestFirst: i + 1,
    cheaper: i > 0 ? rows[i - 1] : null,
    dearer: i < rows.length - 1 ? rows[i + 1] : null,
    value: own.value,
  };
}
