...
import { citiesWithClimate } from './climate.js';
...
export function entityPools(force = false) {
  ...
  pools = {
    ...
    // The cities the climate source measured, which is what the sport season
    // scope counts against rather than every city in the store.
    climateCities: citiesWithClimate(),
...
export function scopeEntities(scopeId, p = entityPools()) {
  ...
  // A city and an activity. The pool is the cities the climate source measured,
  // times the activities the bands are defined for, because a pair whose city
  // has no normals cannot be a page however much demand it has.
  if (s.entity === 'city-activity') {
    return p.climateCities.flatMap((id) => ACTIVITY_IDS.map((a) => id + ':' + a));
  }
