...
export const FILTERS = {
  ...
  // Added 2026-09-25 for the destination matcher, which was specified with a
  // climate filter and had none. The count is months whose comfort score clears
  // the threshold across the measured cities of that country, so a country with
  // one mild city and one extreme one scores lower than either would alone.
  // Twenty three countries have measured cities; the rest cannot pass a climate
  // filter at all and the tool says so rather than guessing at a latitude.
  minComfortableMonths: {
    label: 'At least this many comfortable months a year',
    source: 'climate-normals-verified',
    unit: 'months',
    missing: 'excludes',
    note: 'Counted over the cities measured in that country, on the same comfort model the Climate pages use. Twenty three countries are covered.',
  },
  maxRentBurden: {
...
// How many months of the year clear the comfort threshold in that country.
// Null rather than zero when the country has no measured city, because those
// are different facts and the filter treats them differently.
const COMFORTABLE_AT = 70;
function comfortableMonths(iso2) {
  const c = climateForCountry(iso2);
  if (!c || !c.ranked?.length) return null;
  return c.ranked.filter((m) => m.score >= COMFORTABLE_AT).length;
}
...
function facts(iso2, now) {
  ...
  return {
    iso2,
    comfortableMonths: comfortableMonths(iso2),
...
export function match(criteria = {}, { unit = 'index-eu27-100', language = 'en', limit = 20, now = new Date() } = {}) {
  ...
  for (const iso2 of pool) {
    ...
    if (!out && criteria.minComfortableMonths != null) {
      if (f.comfortableMonths == null || f.comfortableMonths < criteria.minComfortableMonths) { rejected.minComfortableMonths++; out = true; }
      else reasons.push(f.comfortableMonths + ' comfortable months a year, at or over your ' + criteria.minComfortableMonths);
    }
    if (out) continue;
    ...
    considered.push({
      ...
      coverage: f.coverage,
      comfortableMonths: f.comfortableMonths,
