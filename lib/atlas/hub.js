...
// The label and path of a section hub, used by the hub page itself and by the
// breadcrumb on every page under it, so the two can never disagree.
export function hubCrumb(locale, kind) {
  return { name: TEXT[kind][locale].h1, href: '/' + locale + '/' + SEGMENTS[kind][locale] + '/' };
}
