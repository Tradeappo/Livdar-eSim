...
const WANTS = {
  ...
  // The ring comes first here and takes most of the block, because it is the
  // only kind that guarantees the family cannot orphan itself: every page
  // links to the siblings that follow it, so every page receives as many as
  // it gives. Adding the family without a quota left it on the fill path,
  // where all seventeen English pages linked to the same first few and eight
  // of them received nothing at all, which is the orphan problem this table
  // exists to prevent.
  'weather.country-best-time': [['sibling-entities', 5], ['same-entity-other-family', 1], ['rankings', 1], ['tools', 1]],
  // The first city family. Its entity is a city and every other family in the
  // cohort is about a country, so `same-entity-other-family` will find nothing
  // for it and the ring has to carry the whole block. The quota says so
  // explicitly rather than letting the fill path discover it.
  'neighbourhoods.city-where-to-stay': [['sibling-entities', 6], ['tools', 1], ['rankings', 1]],
  // A country family, so the same entity carries a cost of living page and a
  // best time page and this links to both. The ring still takes most of the
  // block, because it is what stops a family orphaning itself.
  'events.country-holidays': [['sibling-entities', 5], ['same-entity-other-family', 2], ['tools', 1]],
  // The regions of one country are each other's closest neighbours, and the
  // ring is what carries them, because no other family in the cohort has a
  // region as its entity.
  'events.subdivision-holidays': [['sibling-entities', 7], ['tools', 1]],
  // A city and an activity. The ring carries most of it for the same reason as
  // the district pages: the entity is a city, and the other city family in the
  // cohort keys on the city alone rather than on the pair, so an exact entity
  // match finds nothing. The tool link is the one that is not a ring link,
  // because a reader deciding when to run is one step from the matcher.
  'sport.city-season': [['sibling-entities', 6], ['tools', 1], ['rankings', 1]],
  // A country family, so the same country carries a cost of living page, a
  // salary page and often a holiday page, and this links to them. The ring is
  // small because there are only five of these pages and a ring of five would
  // otherwise be the whole block.
  'rents.country-inflation': [['same-entity-other-family', 3], ['sibling-entities', 3], ['tools', 2]],
};

// A family with no quota falls through to the fill path, and the fill path
// links every page to the same first few. That is how a family silently
// orphans itself, so the cohort builder asks this first.
export const familiesWithoutQuota = (families) => families.filter((f) => !WANTS[f]);
...
export function linksFor(model, byLanguage, rankedEntities) {
  ...
  //
  // The fill is a ring for the same reason every other kind is one. Walking
  // `bucket.all` in its own order sent every fill slot in a language to
  // whichever page happened to sort first, and the page nobody's quota
  // reached stayed at zero inbound links: the only English holiday page in
  // the cohort was an orphan for exactly that reason.
  if (out.length < MAX_LINKS) {
    for (const target of ring(bucket.all, model)) {
