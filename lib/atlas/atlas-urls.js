...
import { splitEntity as splitSportEntity } from './sport.js';
...
export const ATLAS_SEGMENTS = {
  ...
  // The annual rent increase. German and Dutch take the word their market
  // actually searches, which is also the word the regulator uses, and the other
  // languages take the phrase rather than a noun nobody types.
  'rent-increase': {
    en: 'rent-increase', de: 'mietpreisentwicklung', it: 'aumento-affitti', es: 'subida-del-alquiler',
    fr: 'augmentation-des-loyers', nl: 'huurverhoging', pl: 'wzrost-czynszu', pt: 'aumento-de-rendas',
    ja: 'rent-trend', 'zh-Hant': 'zujin',
  },
  // Sport is the same word in seven of these languages, and a shared segment is
  // not a collision: one route folder serves every language whose segment is
  // that folder, and the language is already the segment above it. Spanish and
  // Portuguese take their own word because neither uses `sport`.
  sport: {
    en: 'sport', de: 'sport', it: 'sport', es: 'deporte',
    fr: 'sport', nl: 'sport', pl: 'sport', pt: 'desporto',
    ja: 'sport', 'zh-Hant': 'yundong',
  },
...
export const FAMILY_SEGMENT = {
  ...
  'rents.country-inflation': 'rent-increase',
};
...
// The slug for a city and an activity. The measured keyword names both, so it
// is used whole, the same way a tool is slugged from the phrase its market
// actually types: `running-in-new-york` in English and `trekking-vicino-milano`
// in Italian, neither a translation of the other. Where the keyword has no
// Latin characters the activity and the city's Latin name are used instead,
// because a percent encoded path is unreadable in a sitemap.
export function sportSlug(entity, language, keyword) {
  const split = splitSportEntity(entity);
  if (!split) return { slug: '', from: 'not a city and activity entity' };
  const fromKeyword = slugify(keyword || '');
  if (isLatinSlug(fromKeyword)) return { slug: fromKeyword, from: 'measured keyword' };
  const city = citySlug(split.cityId, language);
  if (!city.slug) return { slug: '', from: 'no latin name for this city' };
  return { slug: slugify(split.activity + ' ' + city.slug), from: 'activity and the city Latin name, the measured keyword has no Latin characters' };
}
...
export function atlasPath(page) {
  ...
  const leaf = scope === 'tool:global' || scope === 'ranking:list'
    ...
    : scope === 'subdivision'
      ...
      : scope === 'city:activity'
        ? sportSlug(entity, language, keyword)
