...
export const SCOPES = {
  ...
  // One page per region that has public holidays of its own: sixteen German
  // states, nineteen Spanish communities, twenty six Swiss cantons and the
  // Italian and French regions that add days. Counted from the source rather
  // than projected, because the source is built.
  subdivision: { entity: 'subdivision' },
  // A city and an activity together. The entity is the pair, because Tokyo in
  // July is comfortable for swimming and hard for running and one page for both
  // would have to average them into something true of neither. Counted from the
  // cities the climate source actually measured times the four activities the
  // bands are defined for, rather than from every city in the store.
  'city:activity': { entity: 'city-activity', climateCities: true, activities: 4 },
...
export const FAMILIES = {
  ...
  // Added 2026-09-25. The Stay surface needed a family whose source is built,
  // and this is the only question the rent series can answer honestly: not what
  // rent costs, which Eurostat does not publish, but how much rents have moved
  // and how that compares with the other thirty six countries. Two markets
  // regulate the annual increase and ask about it every year, which is why the
  // Dutch and French numbers are the largest in the surface.
  'rents.country-inflation': {
    vertical: 'rents', axis: 'audience', scope: 'country', variants: 1,
    intent: 'How much rents have risen in this country, and where that sits against the rest',
    requiredSources: ['rent-index-verified'], minWords: 300,
    uniqueness: 'this country index against 2015, its annual change and its rank among the thirty six',
    measured: { pattern: 'rent increase {year}', bestVolume: 4400, medianVolume: 250, difficulty: 0, market: 'nl-NL', on: '2026-09-25' },
    evidence: { source: 'data/atlas/measurements/ahrefs-rent-2026-09-25.json', note: 'huurverhoging 2026 is 4,400 a month at difficulty 0 and irl 2026 is 4,300 at difficulty 18. Both markets regulate the annual increase by a published index.' },
    evidenceState: 'measured',
    leadsTo: ['rental-lead'],
    aeo: ['direct-answer', 'fact-table'],
    indexPolicy: 'always, for a country in the European series',
    ...v(0.6, 0.85, 0.65, 0.8, 0.75, 0.8),
  },
  'rents.neighbourhood': {
  ...
  // Added 2026-09-24. Pulse had the largest measured demand of any surface
  // and no source, because every event feed worth having is licensed, behind
  // a portal this container cannot reach, or a scrape the brief forbids.
  // Public holidays are the exception and they are not a consolation prize:
  // `feriados 2026` is 705,000 searches a month in Brazil, `feestdagen 2026`
  // is 95,000 in the Netherlands and `jours feries 2026` is 181,000 in France
  // at difficulty 3. They recur, the state publishes them, and the dates are
  // known years ahead, which is exactly the shape this family wants.
  'events.country-holidays': {
    vertical: 'events', axis: 'language', scope: 'country', variants: 1,
    intent: 'Which days are public holidays in this country, which of them the whole country keeps, and which fall where they are worth a long weekend',
    requiredSources: ['events-verified'], minWords: 300,
    uniqueness: 'this country dated list for this year, split into the days kept everywhere and the days kept only in named regions',
    evidence: { source: 'data/atlas/probes/holidays-2026-09-24.json', note: 'measured 2026-09-24 in nine markets. feriados 2026 at 705,000 in pt-BR, jours feries 2026 at 181,000 and difficulty 3 in fr-FR, feestdagen 2026 at 95,000 in nl-NL, feiertage 2026 at 90,000 in de-DE, dni wolne od pracy 2026 at 67,000 in pl-PL' },
    evidenceState: 'measured',
    lifecycleShape: 'recurring-event',
    leadsTo: ['stay-booking', 'ticket'],
    aeo: ['direct-answer', 'dated-list'],
    indexPolicy: 'always',
    ...v(0.45, 0.7, 0.4, 0.75, 0.7, 0.95),
  },
  ...
  // is 48,000 against 10,000 for Spain. The reason is the same in both: the
  // days are set by the region, so the national list is the wrong answer for
  // most readers and they know it.
  'events.subdivision-holidays': {
    vertical: 'events', axis: 'language', scope: 'subdivision', variants: 1,
    intent: 'Which days are public holidays in this region, and how many more than the rest of the country gets',
    requiredSources: ['events-verified'], minWords: 280,
    uniqueness: 'this region dated list against its own country, which is the comparison the national page cannot make',
    evidence: { source: 'data/atlas/probes/holidays-2026-09-24.json', note: 'measured 2026-09-24. feiertage nrw 2026 at 202,000 and difficulty 1, feiertage bayern 2026 at 149,000, festivos madrid 2026 at 48,000, all of them larger than their own national term' },
    evidenceState: 'measured',
    lifecycleShape: 'recurring-event',
    leadsTo: ['stay-booking', 'ticket'],
    aeo: ['direct-answer', 'dated-list'],
  ...
  // Added 2026-09-25. The Sport surface had two families and both need the
  // route extract that this environment cannot reach, so the surface carried no
  // page at all. This one is the half of the question the built data answers:
  // the months an outdoor activity is comfortable in a city, from the same
  // normals the Climate surface already uses, with the routes named as the part
  // it does not hold.
  'sport.city-season': {
    vertical: 'sport', axis: 'language', scope: 'city:activity', variants: 4,
    intent: 'When this activity is comfortable in this city, month by month',
    requiredSources: ['geonames-cities', 'climate-normals-verified'], minWords: 300,
    uniqueness: 'the comfortable band for this activity against this city measured months',
    measured: { pattern: '{activity} in {city}', bestVolume: 500, medianVolume: 90, difficulty: 0, market: 'ja-JP', on: '2026-09-25' },
    evidence: { source: 'data/atlas/measurements/ahrefs-sport-2026-09-25.json', note: 'trekking vicino milano 350 at difficulty 0, senderismo barcelona 450 at difficulty 0, 東京 ハイキング 500 at difficulty 0. Twenty nine city and activity pairs carry demand across six markets.' },
    evidenceState: 'measured',
    leadsTo: ['sport-rental', 'stay-booking', 'esim-sale'],
    aeo: ['direct-answer', 'fact-table'],
    indexPolicy: 'always, for a city the climate source measured and an activity the bands cover',
    ...v(0.6, 0.75, 0.6, 0.85, 0.75, 0.8),
  },
  'sport.route': {
  ...
  'transport.route-from-market': {
    ...
    evidence: { source: 'data/atlas/probes/transport-2026-09-24.json', note: 'measured 2026-09-24, en-US: difficulty 0 and 1 across the phrasings tested, and 0 to 150 searches a month. Cheap to rank for and worth almost nothing.' },
    evidenceState: 'measured',
    keptBecause: 'It needs no source that is not built, so it stays available. It is not worth a cohort slot: the demand was measured and it is close to zero, which is a different answer from being blocked.',
    leadsTo: ['esim-sale', 'stay-booking'],
    aeo: ['direct-answer', 'fact-table'],
    indexPolicy: 'only where the route carries measured demand, which on this evidence is nowhere yet',
  ...
  'weather.country-best-time': {
    ...
    requiredSources: ['geonames-cities', 'climate-normals-verified'], minWords: 340,
  ...
  'airports.guide': {
    ...
    evidence: { source: 'data/atlas/probes/transport-2026-09-24.json', note: 'measured 2026-09-24, en-US: real volume, prohibitive difficulty. Median 43 over fifteen airports, against about 2 for the best time family measured the same day. Split 93, Dubrovnik 88, Narita 71, Changi 60. The only easy ones are Seville at 2 and Tromso at 0, carrying 600 and 450 a month.' },
    evidenceState: 'measured',
    // The earlier note said named airport queries have real volume, and that
    // is true and was the wrong test. The airport's own site holds the query,
    // and a page of identifiers, position and elevation is not what displaces
    // it. This is the same mistake as ranking a family on volume alone, which
    // the prioritisation already refuses to do elsewhere.
    keptBecause: 'It needs no source that is not built, so it stays available for a page that has something the airport site does not. It is not worth a cohort slot on the identifier content alone.',
    leadsTo: ['esim-sale', 'stay-booking'],
    aeo: ['direct-answer', 'fact-table'],
    indexPolicy: 'only the airports where the query is not already held by the operator, which the measurement says is a small minority',
