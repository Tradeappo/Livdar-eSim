...
// The countries immediately above and below on the same series.
//
// A rank and a median say where a country sits; two names say what that
// position feels like. It is also what separates two pages whose numbers are
// close: Denmark and Sweden differ by a rank and a figure, and by which
// countries stand on either side of them.
export function neighbours(iso2, measure = 'netMonthly') {
  const rows = Object.entries(store().countries)
    .map(([k, c]) => ({ iso2: k, value: c.measures[measure]?.value }))
    .filter((r) => typeof r.value === 'number')
    .sort((a, b) => b.value - a.value);
  ...
  return {
    measure,
    value: rows[i].value,
    above: i > 0 ? rows[i - 1] : null,
    below: i < rows.length - 1 ? rows[i + 1] : null,
    of: rows.length,
  };
}
