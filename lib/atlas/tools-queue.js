...
export const TOOLS = [
  {
    id: 'moving-cost', implementation: 'lib/atlas/tools/moving-cost.js', name: 'Moving cost calculator', family: 'tools.calculator',
  ...
  {
    id: 'where-should-i-live', implementation: 'lib/atlas/tools/matcher.js', name: 'Where should I live', family: 'tools.matcher',
  ...
  {
    id: 'rent-affordability', implementation: 'lib/atlas/tools/rent-affordability.js', name: 'Rent affordability', family: 'tools.calculator',
    volume: 3600, difficulty: 31, cpcUsd: 0.3,
    markets: ['all eleven, unmeasured outside en-US'],
    competitor: 'US rental portals, almost entirely domestic',
    inputs: ['income', 'household size'],
    sources: ['rent-index-verified'],
    leadsTo: ['rental-lead'],
    complexity: 'small',
    note: 'Reclassified on 2026-09-25. It was waiting for rent-level-verified on the grounds that a rent index cannot price a city, which is true and was the wrong reason to hold the tool: the question is what share of an income should go on rent, and that is answered from the income and the rule, not from the rent. So the tool computes the budget, names the rule it used, and says out loud that it cannot tell you whether that budget is enough in a particular city. The city input was removed for the same reason, because an input the tool cannot honour is a promise it breaks.',
  },
  {
    id: 'cost-of-living-comparison', implementation: 'lib/atlas/tools/cost-of-living.js', name: 'Cost of living comparison', family: 'tools.cost-comparison',
  ...
  {
    id: 'relocation-budget', implementation: 'lib/atlas/tools/relocation-budget.js', name: 'Relocation budget', family: 'tools.calculator',
  ...
  {
    id: 'travel-budget', implementation: 'lib/atlas/tools/cost-of-living.js', name: 'Travel budget', family: 'tools.calculator',
  ...
  },
  {
    ...
    // `rent-index-verified` was listed here and it is the wrong source. It is
    // a country level index of how rents have moved, and this tool has to
    // filter the districts of one city by what they cost. A country number
    // cannot do that for any of them. When the neighbourhood source was built
    // the queue therefore declared this tool ready while two of its four
    // inputs had nothing behind them, which is the failure the readiness test
    // exists to catch and it caught it.
    sources: ['neighbourhood-facts-verified', 'rent-city-verified'],
    leadsTo: ['rental-lead', 'stay-booking'],
    complexity: 'medium',
    note: 'No measured demand for the tool itself. It is here because it is the persona axis made interactive, and because the neighbourhood pages it sits on do have measured demand in every market. The district positions exist now; the budget and persona inputs wait on city level rent.',
  },
  {
    id: 'city-comparison', implementation: 'lib/atlas/tools/cost-of-living.js', name: 'City comparison', family: 'tools.cost-comparison',
    volume: null, difficulty: null, cpcUsd: null,
    markets: ['all eleven'],
  ...
  {
    id: 'country-comparison', implementation: 'lib/atlas/tools/country-comparison.js', name: 'Country comparison', family: 'tools.cost-comparison',
    volume: null, difficulty: null, cpcUsd: null,
    markets: ['all eleven'],
    competitor: 'livingcost, whose British pages are sixteen of its top twenty',
    inputs: ['country a', 'country b'],
    sources: ['cost-of-living-verified', 'rent-index-verified', 'salary-data-verified'],
    leadsTo: ['relocation-service'],
    complexity: 'small',
    note: 'Respecified on 2026-09-25. Tax rules are not built and the comparison does not need them to be useful: price level, rent movement and average earnings are three measures that exist for both sides on one definition each. The tool compares those three and leaves the tax row visibly absent rather than estimating it, which is the same rule the city comparison already follows for a measure one side is missing.',
  },
  {
  ...
  {
    id: 'destination-matcher', implementation: 'lib/atlas/tools/matcher.js', name: 'Remote work destination matcher', family: 'tools.matcher',
    volume: null, difficulty: null, cpcUsd: null,
    markets: ['all eleven'],
    competitor: 'nomads.com, whose city profiles take between 10 and 47 visits each',
    inputs: ['budget', 'climate'],
    sources: ['cost-of-living-verified', 'climate-normals-verified'],
    leadsTo: ['premium-tool', 'relocation-service'],
    complexity: 'medium',
    note: 'The competitor is explicit negative evidence, which is a reason to build the tool and not the thousand city pages behind it. Narrowed on 2026-09-25 to the two filters there is data for. It was specified with a timezone and a visa filter and neither dataset exists, so a reader who picked a visa status would have been answered out of nothing. Budget comes from the price level and climate from the monthly normals, and the tool says which two questions it is not answering.',
  },
  {
    id: 'where-should-i-stay', implementation: 'lib/atlas/tools/stay-matcher.js', name: 'Where should I stay', family: 'tools.matcher',
    volume: null, difficulty: null, cpcUsd: 0.3,
    markets: ['all eleven'],
    competitor: 'booking sites answer availability, not which area',
    inputs: ['city', 'anchor'],
    sources: ['neighbourhood-facts-verified'],
    leadsTo: ['stay-booking'],
    complexity: 'medium',
    note: 'Best areas to stay in Rome is 700 a month at difficulty 3, which is the page this tool sits on. Narrowed on 2026-09-25 to the half that has data. The district geography is built and the inventory is not, so the tool ranks districts by where they are rather than by what a night costs in them, and the budget and trip purpose inputs were removed because both would have needed the inventory to mean anything.',
  },
  {
  ...
  {
    id: 'salary-calculator', implementation: 'lib/atlas/tools/salary.js', name: 'Salary calculator', family: 'tools.calculator',
  ...
  {
    id: 'cost-of-living-calculator', implementation: 'lib/atlas/tools/cost-of-living.js', name: 'Cost of living calculator', family: 'tools.cost-calculator',
    volume: 39000, difficulty: 78, cpcUsd: 0.45,
    markets: ['measured in all ten cohort markets on 2026-09-24'],
    competitor: 'numbeo and the US salary comparison calculators',
    inputs: ['city', 'household size', 'income'],
    sources: ['cost-of-living-verified'],
    leadsTo: ['relocation-service', 'premium-tool'],
    complexity: 'medium',
    note: 'A different page from the comparison, and the distinction is the whole reason it has its own family. The comparison answers city A against city B. The calculator answers what one place costs for one household, which is the question somebody asks once they have already chosen.',
  },
  {
...
// Tools that exist as code rather than as a specification. The brief was
// explicit that a buildable tool left unbuilt is not progress, so the two
// states are separate: `sourcesReady` says it could be built, `built` says
// somebody did.
export const built = () => TOOLS.filter((t) => t.implementation).map((t) => t.id);
export const unbuiltButBuildable = (blockedSources = new Set()) =>
  buildable(blockedSources).filter((t) => !t.implementation).map((t) => t.id);
