...
import { publishedDestinationIds, publishedGuideSlugs, guideContent, publishedRegionIds, regionContent, compatibilityContent, legalContent, LEGAL_KINDS } from './content/index.js';
...
export function resolvePath(locale, slugParts) {
  ...
  if (first === segment('regions', locale)) {
    ...
    if (parts.length === 2) {
      ...
      const content = regionContent(locale, region.id);
      if (!content) return null;
      return { type: 'region', locale, region, content };
    }
    return null;
  }
  ...
  if (parts.length === 1) {
    for (const kind of LEGAL_KINDS) {
      if (first === segment(kind, locale)) {
        const content = legalContent(locale, kind);
        if (!content) return null;
        return { type: 'legal', locale, kind, content };
      }
    }
  }

  if (first === segment('compatibility', locale) && parts.length === 1) {
    const content = compatibilityContent(locale);
    if (!content) return null;
    return { type: 'compatibility', locale, content };
  }
...
export function allPathsForLocale(locale) {
  ...
  publishedRegionIds(locale).forEach((id) => out.push([segment('regions', locale), id]));
  out.push([segment('guides', locale)]);
  publishedGuideSlugs(locale).forEach((slug) => out.push([segment('guides', locale), slug]));
  if (compatibilityContent(locale)) out.push([segment('compatibility', locale)]);
  LEGAL_KINDS.forEach((kind) => {
    if (legalContent(locale, kind)) out.push([segment(kind, locale)]);
  });
  return out;
}
