...
import registry from '../../data/publication-registry.json' with { type: 'json' };

// Authored content is necessary but not sufficient. A destination, guide or
// region page is routed only when the publication registry marks it
// "published". Content written but not yet approved stays out of routing,
// sitemaps, hreflang and llms files; flipping an entry to "retired" is the
// rollback, and it needs no content to be deleted.
export const PUBLICATION_REGISTRY = registry;
export function publicationState(locale, type, id, source = registry) {
  const entry = source.entries[locale + ':' + type + ':' + id];
  return entry ? entry.state : 'unregistered';
}
export function gate(tables, type, source = registry) {
  const out = {};
  Object.entries(tables).forEach(([locale, table]) => {
    out[locale] = Object.fromEntries(
      Object.entries(table).filter(([id]) => publicationState(locale, type, id, source) === 'published')
    );
  });
  return out;
}
const AUTHORED = {};
export function authoredIds(locale, type) {
  return Object.keys((AUTHORED[type] || {})[locale] || {});
}
export function authoredContent(locale, type, id) {
  return ((AUTHORED[type] || {})[locale] || {})[id] || null;
}
...
const DESTINATION_CONTENT = gate(AUTHORED.destination, 'destination');
...
const REGION_CONTENT = gate(AUTHORED.region, 'region');
...
const GUIDE_CONTENT = gate(AUTHORED.guide, 'guide');
